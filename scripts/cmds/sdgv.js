const games = global.tcccGames || (global.tcccGames = new Map());
...
function getRandomQuestion(game) {
  let available = WORDS
    ...
    .filter(x => !game.used.includes(x.index));

  if (available.length === 0) {
    game.used = [];

    available = WORDS.map((item, index) => ({
      item,
      index
    }));
  }
...
🔤 كانت كتبدأ بحرف:
${cleanWord(current.word).charAt(0)}
...
module.exports = {
  ...
  config: {
    ...
    countDown: 3,
    role: 0,
    shortDescription: "لعبة استخراج الكلمات",
    category: "games"
  },
...
🔄 كتب tccc باش تعاود تبدأ.`,
        threadID
      );
    }

    /*
     * منع تشغيل لعبة ثانية
     */

    if (games.has(threadID)) {

      return api.sendMessage(
        "⚠️ اللعبة راه خدامة دابا!\n🛑 كتب tccc of باش توقفها.",
        threadID
      );
    }
...
🔤 كل جولة نعطيك الحرف الأول

🥇 أول واحد يلقى الكلمة
⭐ يربح نقطة

🔀 الفئات مختلطة وعشوائية
...
🔄 كتب tccc باش تعاود تبدأ.`,
    ...
    /*
     * قفل الجولة باش غير أول واحد يربح
     */
